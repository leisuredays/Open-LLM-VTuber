@echo off
REM Open-LLM-VTuber Desktop App Build Script for Windows (Without Code Signing)
...
echo.
echo [3/4] Building Windows executable (without code signing)...
call npm run build
if %errorlevel% neq 0 (
...
call npx electron-builder --win --x64 --config.win.sign=null
if %errorlevel% neq 0 (
